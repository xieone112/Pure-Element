...
| type | 原生类型，支持 text 和 textarea 等 | `string` | text |
...
| change | 失焦且值变更后触发 | `(value: string) => void` |
...
| 方法名 | 说明 | 类型 |
| --- | --- | --- |
| ref | 原生输入框节点（input 或 textarea） | `HTMLInputElement \| HTMLTextAreaElement` |
